...
data <- data %>%
  ...
  dplyr::mutate(
    dplyr::across(
      dplyr::any_of(c("age_group","Season","Period","net","fumig","centro","gravidity","sede","hiv", "gestweek")),
      as.factor
    )
  ) %>%
...
covars <- c("age_group", "Season", "Period", "gestweeks", "net", "fumig", "sede", "hiv", "gestweek")
...
rhs <- paste0("c_log_density * gravidity * centro + ", " + ", covar_str)
...
write.csv(
  out,
  "_MODEL_RESULTS_HRP2_PfLDH_PCRpos_CommonSupport.csv",
  row.names = FALSE
)
...
p_levels <- make_panel_forest(lev_hrp2, "HRP2 levels (Primi anchored vs all)", burgundy, TRUE, ylim_max_levels) +
  make_panel_forest(lev_ldh,  "PfLDH levels (Primi anchored vs all)", blue, FALSE, ylim_max_levels) +
...
p_slopes <- make_panel_forest(sl_hrp2, "HRP2 slopes (Primi anchored vs all)", burgundy, TRUE, ylim_max_slopes) +
  make_panel_forest(sl_ldh,  "PfLDH slopes (Primi anchored vs all)", blue, FALSE, ylim_max_slopes) +
  patchwork::plot_layout(widths = c(1, 1))
...
run_hasta_desde_primi_anchor_vs_all <- function(model_obj, marker_tag, colour_hex, y_expr, unit_suffix,
  ...
  cat("\n================ HASTA/DESDE:", marker_tag, "(Primi anchored vs all) ================\n")
  ...
  p_IJ <- plot_contrast_curves(
    curves_IJ, labels_IJ,
    title_txt  = paste0("Hasta/Desde — ", marker_tag, " outcome (Primi anchored vs all) — IJ facets"),
...
res_hasta_hrp2  <- run_hasta_desde_primi_anchor_vs_all(
  ...
  marker_tag  = "HRP2",
  colour_hex  = burgundy,
  y_expr      = expression(Delta*log(1+HRP2)),
  unit_suffix = "p/µL"
)
...
res_hasta_pfldh <- run_hasta_desde_primi_anchor_vs_all(
  ...
  marker_tag  = "PfLDH",
  colour_hex  = blue,
  y_expr      = expression(Delta*log(1+PfLDH)),
  unit_suffix = "p/µL"
)
